...
static void chart_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_PRESSED || code == LV_EVENT_RELEASED) {
        ...
        if(add_value) {
            ...
            // Fix build error
            #if defined(REMOVE_LVGL_ORIGINAL_SETTING) && (REMOVE_LVGL_ORIGINAL_SETTING == 1)
            lv_snprintf(buf, sizeof(buf), "%d", lv_chart_get_series_y_array(obj, (lv_chart_series_t *)ser)[base_dsc->id2]);
            #else
...
static void scale2_timer_cb(lv_timer_t * timer)
{
    ...
    lv_label_set_text_fmt(label, "Desktop: %u", session_desktop);
    label = lv_obj_get_child(card, -3);
    lv_label_set_text_fmt(label, "Desktop: %u", session_tablet);
    label = lv_obj_get_child(card, -1);
    lv_label_set_text_fmt(label, "Desktop: %u", session_mobile);
    #else
    label = lv_obj_get_child(card, -5);
    lv_label_set_text_fmt(label, "Desktop: %"LV_PRIu32, session_desktop);
    label = lv_obj_get_child(card, -3);
    lv_label_set_text_fmt(label, "Desktop: %"LV_PRIu32, session_tablet);
    label = lv_obj_get_child(card, -1);
    lv_label_set_text_fmt(label, "Desktop: %"LV_PRIu32, session_mobile);
    #endif
    // <Sam_Diao20250607->
}
